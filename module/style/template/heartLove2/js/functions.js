...
function getHeartPoint(c) {
	...
	return new Array(offsetX + a, 690/2 -55 + d);
...
function adjustWordsPosition() {
    ...
    $("#words").css("top", $("#garden").position().top + 200);
    $("#words").css("left", $("#garden").position().left + 70)
}
function showLoveU() {
    $("#loveu").fadeIn(3000);
};
function setCodeMargin(){
	var codeHeight = $('#code').height();
	var winHeight = $(window).height();
	$('#code').css('padding-top', Math.max((winHeight - codeHeight)/2 - 30, 30) + "px");
}
